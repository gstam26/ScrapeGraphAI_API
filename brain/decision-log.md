...
## 2026-08-03 — SINGLE-BEST A/B CONFIRMED 3× STRONGER THAN PREVIEW → DEFAULT ON; FINAL GT (59 entities) SCORED: F1 0.606; matcher qualified-numeric fix; NLI equivalence probe (swap gated)

**1. Real-render single-best A/B (laptop, warm-cache re-render, 7m48s, 53 fresh LLM calls / 1,598 cached).** Scored vs gt42, matrix+prose, both legs same matcher: as-shipped P 0.492 / R 0.611 / F1 0.545 → single-best **P 0.572 / R 0.582 / F1 0.577** — P +0.080, F1 +0.032, hallucination 0.508→0.428, at R −0.029 (24 lead-pick errors, the pre-named residual class). The counterfactual preview (+0.028 P) under-predicted 3×: the real render also collapses redundancy (83→23) and display-incoherent cells ("Yes" + "Not disclosed" stacked — the Asteelflash systems-integration pattern George spotted; suppressed-null 187→88). Biggest wins exactly where predicted: employees P 0.49→0.65, HQ P 0.39→0.76, acquirer, revenue. **DECISION (George): `MATRIX_SINGLE_BEST` default ON** (config.py; env `"false"` reproduces old renders; 3 legacy tests pinned flag-off — flag-on coverage lives in test_matrix_single_best.py). Cross-machine note: the as-shipped leg reproduced the Dell scoring EXACTLY (TP/FN/FP 503/320/519) — the eval stack (CE included) is machine-reproducible.

**2. Detail-sheet audit (both grains) → one real matcher bug + one GT typo; sheet format exonerated.** The "claim vs blank" rows George questioned are the writer's row-union format (gt-side and ai_only rows of the same cell are separate lines; all 160 gt-null auto_misses have their assertion elsewhere in-cell — recorded 07-29 wart, not mis-scoring). Real bug: **qualified numerics** — `_numeric_value` parsed only PURE numbers, so '6500+' vs '6,500+' fell to the CE, which scores short number strings ~0 → veto → FN+FP for identical values (3 cells: Phillips Medisize, Plexus, Mack). FIX b12224f: strip non-value-changing qualifiers (trailing +, over/at least/approx/~) before the typed equal-or-nothing path; ranges/units still parse None. Plus Maenner GT 'Ye'→'Yes' — the analyst independently made the same fix in the final template (convergent). Morocco/Monaco at CE 0.000 confirmed the veto EARNING its keep on lookalikes.

**3. NLI equivalence probe (George challenged the strip as too manual — measured, not argued).** nli-deberta-v3-base, bidirectional-entailment-min, local cache: fixes the paraphrase-veto class ms-marco fails (Asteelflash prototyping↔mass-production 0.986 vs ~0; 'Ye'/'Yes' 0.980) and keeps every veto win (Morocco/Monaco, Yes/No, 2003/2004, Tornos/Rentas all ≤0.02). BUT task1 label agreement 0.900 < ms-marco's 0.967 (three subject-less mission-prose pairs → the verify-tier context-blindness, same fix candidate: context premise) — and it REFUSES '20,000+' vs '20,000' (0.041) on principled strictness, **vindicating the typed rule: qualifier-equivalence is our GT norm, not a semantic fact — norms live in deterministic rules.** DECISION: no swap on the labels it just failed; gate = second label set (CMO pairs, George labels) with the context-premise variant. Azure LLM judge stays rejected for the ruler (non-determinism measured 07-24; no key on Dell breaks cross-machine reproduction; volume).

**4. FINAL GT LANDED (CH_JH_IA): 42→59 substantive entities, triaged, converted, scored.** Third analyst filled R–Z (17 entities, conventions CLEAN by gt_convention_check — zero divergence flags, is_list identical to gt42). Staged copy blanks 8 nav-note cells: 4 duplicates (Integer/Lake Region→Greatbatch; Nypo→Jabil; Flextronics Intl→Flextronics), 3 access failures (Kitron — pipeline ALSO failed it same day, mutual; Hans Koch domain-for-sale; Medifiq site gone), and **KMC Systems: seed redirects to hiarc.inc, a DIFFERENT company (Acquire Log, 16 wrong-company pages) — analyst's "website does not correlate?" confirmed, excluded as identity-failure finding.** **Sanmina reversal recorded: analyst browser-blocked ("forbidden"), pipeline fetched 31 pages / answered 16 cells** — the tool reads a site the analyst couldn't; Valtronic partial (1 splash page vs analyst fully blocked). Gaudlitz + KingField KEPT with full GT = ~34 honest bot-wall FNs (recall price of the open UA-policy decision). Converted: `outputs/cmo_gt_final59.xlsx`, 1,158 GT rows, 384 nulls, round-trip verified.

**5. HEADLINE (single-best render vs final GT, matrix+prose, fixed matcher): P 0.596 / R 0.617 / F1 0.606, hallucination 0.405** (1,003 cells / 59 entities). First ≥0.6. New R–Z cohort scores ABOVE the old block (reachable sites: description 0.824, revenue 0.818, employees 0.692 — numeric fix visible). Floor unchanged = the norm-class questions (volume 0.226, low-volumes 0.336, PCB 0.396) → instructions experiment remains THE lever. Reproduced on the laptop to the third decimal via the new one-command runner (`scripts/score_cmo.py`: bakes matrix+prose+CE headline config, styled report with Run Info provenance sheet, verdict colour-coding). New tools this session: `gt_convention_check.py` (analyst-fill divergence, null-vs-blank is the precision-mover — blank cells on covered entities can only charge FP, verified empirically), `gt_agreement.py` (IAA via the eval's own matcher; same-annotator reference 0.986). Suite 320.
...
**Pre-registered scorecard (entry below):** Expectation 1 — 0/3 weak-evidence Yes cells flipped; Adapt low-volumes Yes re-extracted from the SAME marketing quote ("huge amount of SMT capacity and flexibility") despite the explicit menu-label/marketing-phrase bar. Expectation 2 — 1/2 (Avenue EOL-No gone ✓; Avenue PCB-No persisted). Expectation 3 — held (exclusively-China 5/5; Asteelflash low-volumes No stayed, the accepted residual). Expectation 4 — middle class persisted untouched. Expectation 5 — FAILED, and worse: three wrong-direction changes, two of which VIOLATE the new rules in fresh output (Adapt tooling Yes NEWLY asserted from a marketing quote; Arrk PCB No NEWLY inferred from 70-years-of-focus wording) plus one real TP lost (Automatic EOL Yes extraction dropped entirely). **Verdict: GPT-4.1-mini does not reliably apply per-question evidence-bar rules inside the 17-question batch prompt — topic-presence → Yes pattern dominates instruction text. Instruction specificity is refuted as the lever for this class** (contrast: format/structure instructions worked s1→s6; judgment norms are a different species).

**NLI-demotion counterfactual (same day, offline on the join workbook):** rule "demote Y/N assertions with quote-support < t to Not disclosed" at t=0.1/0.3/0.5 fixes 3 assert-vs-ND errors but destroys 5–6 REAL TPs at every threshold (Automatic EOL Yes support 0.004 yet GT says Yes; Asteelflash medical 0.010 ditto) — weak evidence does not imply wrong answer. Demotion refuted; NLI stays annotate-only.

**Standing decision:** the assert-vs-not-disclosed boundary (~19 cells on 5 entities) is ACCEPTED as measured residual after three pre-registered attacks all refuted with mechanisms (error-prediction, instruction-specificity, support-demotion). It is substantially genuine norm disagreement + extraction noise, not a fixable defect at current model tier. Report it as such; Asteelflash EOL is the adjudication exhibit (v3's quotes — "AOI to verify component placement", "full assembly...testing" — are arguably BETTER evidence than the GT's conservative Not-disclosed). Remaining ladder rung = model escalation (George's call; not recommended on cost/deployability). **The dominant measured lever is now ACQUIRE STARVATION (17/25 single FN from 2 entities with 2–3 pages) and simply more GT coverage.** The three-refutations arc is strong dissertation material — keep all artifacts (cmo_eval_matrix_v3.xlsx, nli_errors_join.xlsx, this chain).
