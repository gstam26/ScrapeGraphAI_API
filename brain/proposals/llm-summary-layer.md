...
## What leadership asked for and what it breaks

Leadership wants readable synthesized prose per cell — the thing the deterministic Digest deliberately isn't (decision-log 2026-07-03: "LLM prose would need a faithfulness eval to earn the same trust"). That trade was recorded as a future option; this design is that option, priced honestly. LLM prose breaks two properties every other layer guarantees:
...
3. **Escalate model choice to George.** Options at that point, cheapest first: a stronger model for the judge only (the judge is the trust-critical role, ~half the calls); a larger Azure deployment if one exists on the resource; back to the GPT-5.5 proxy for both roles — reopening the determinism and throughput trade this revision closed.
...
3. **Sanction:** Azure-direct is the approved LLM path now. The previously recorded compliance position was "the only LLM available on-network is the Power Automate proxy" (`semantic-verify.md`, 2026-07-02) — George's instruction reverses that; worth having the reversal confirmed in writing (IT) since it's a policy change, not just a config change.
